    ...
    def get_desc(lang: str = "en"):
        ...
        if lang == "zh":
            return (
                ...
                "输入参数：\n"
                "- input_key: 包含原始指令的字段名 (默认: 'messages')\n"
                "输出参数：\n"
                "- output_key: 用于存储生成指令的字段名 (默认: 'generated_instruction')\n"
            )
        else: # Default to English
            return (
                "This operator enhances human instructions by unifying tasks with different output formats into requests for generating complete functions.\n\n"
                "Input Parameters:\n"
                "- input_key: Field name containing the original instruction (default: 'messages')\n"
                "Output Parameters:\n"
