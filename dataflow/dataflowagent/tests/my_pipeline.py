...
class RecommendPipeline(PipelineABC):
    def __init__(self):
        ...
        self.reasoning_answer_generator = ReasoningAnswerGenerator(
            llm_serving=self.llm_serving, prompt_template=None
        )
        self.reasoning_question_generator = ReasoningQuestionGenerator(
            num_prompts=1, llm_serving=self.llm_serving, prompt_template=None
        )

    def forward(self):
        # 1. Generate chain-of-thought (CoT) answers based on the existing "question" field
        self.reasoning_answer_generator.run(
            storage=self.storage.step(), input_key="question", output_key="generated_cot"
        )

        # 2. Generate follow-up questions using the freshly created "generated_cot" column
        self.reasoning_question_generator.run(
            storage=self.storage.step(),
            input_key="generated_cot",  # Fixed: provide a valid existing column
